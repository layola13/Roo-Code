		...
		closeBundle: {
			...
			handler: async () => {
				try {
						...
						jsFiles.map(async (jsFile) => {
							...
							if (mapExists) {
								...
								if (!jsContent.includes("//# sourceMappingURL=")) {
									...
									// Write the updated JS file - async
									await fs.writeFile(jsPath, jsContent)
								}

								// Make sure map file is in the correct format and has proper sourceRoot
								try {
									const mapContentStr = await fs.readFile(mapPath, "utf8")
									const mapContent = JSON.parse(mapContentStr)
									...
									// Write back the updated source map with proper formatting - async
									await fs.writeFile(mapPath, JSON.stringify(mapContent, null, 2))
					...
					// Create a special file to enable source map loading in production - async
					await fs.writeFile(
					...
					console.error("Error in sourcemapPlugin:", error)
					// Don't throw - allow build to complete even if source map processing fails
				}
			},
		},
	}
