...
- When using the read_file tool, be aware that files over 180 KB MUST be read with line_range OR split using the split_file tool. For large files (especially webpack/minified files), use the split_file tool first to break them into manageable chunks. You can also use list_code_definition_names to understand structure, then read specific sections with line_range (maximum 1500 lines per range). Never use manual PowerShell or bash commands to split files - always use the split_file tool instead.
