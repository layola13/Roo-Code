...
- You have the ability to execute multiple independent tasks in parallel using the spawn_parallel_tasks tool. This is particularly powerful for:
  * Modifying multiple independent files simultaneously (e.g., updating multiple components that don't depend on each other)
  * Analyzing different parts of a large codebase in parallel (e.g., reviewing multiple modules for security issues)
  * Running independent operations that don't share state (e.g., generating documentation for multiple packages)
  * Performing batch operations where tasks can be completed independently
  When you identify tasks that are independent and can be executed concurrently, you should proactively use this tool to improve efficiency. The system supports up to 10 concurrent tasks by default, with real-time progress tracking and automatic result aggregation.${
		supportsComputerUse
