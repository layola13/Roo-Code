 ...
 * @param {Task} task - Optional Task instance (required if using subagent compression)
 * @returns {SummarizeResponse} - The result of the summarization operation (see above)
 */
export async function summarizeConversation(
	...
	if (subAgentConfig && task && shouldUseSubAgentCompression(subAgentConfig)) {
		try {
			// Execute subagent compression
			const subAgentResult = await executeSubAgentCompression(messages, subAgentConfig, task)
